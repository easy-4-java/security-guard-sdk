...
Project scaffold for a security guard / anti-abuse SDK. As of the `3.0.x.x.20260630-SNAPSHOT` snapshot the repository contains the Maven project skeleton only — no production code has been added yet.
...
- It is not a usable library yet: the project contains **no `src/main/java` sources** at the `3.0.x.x.20260630-SNAPSHOT` snapshot.
...
```groovy
implementation 'io.github.easy4j:security-guard-sdk:3.0.x.x.20260630-SNAPSHOT'
```
...
**Expected result:** the build succeeds and installs an (effectively empty) `security-guard-sdk-3.0.x.x.20260630-SNAPSHOT.jar` into the local repository.
